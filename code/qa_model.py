...
class QASystem(object):
    def __init__(self, encoder, decoder, embed, vocab, *args):
        """
        ...
        with tf.variable_scope("qa", initializer=tf.contrib.layers.xavier_initializer()):
            #self.setup_embeddings() #used later in system_setup
    ...
    def setup_system(self):
        ...
        cell0 = tf.contrib.rnn.GRUBlockCell(cell_size=FLAGS.state_size)
        cell1 = tf.contrib.rnn.GRUBlockCell(cell_size=FLAGS.state_size)
        cell2 = tf.contrib.rnn.GRUBlockCell(cell_size=FLAGS.state_size)  # ,input_size=FLAGS.embedding_size)
        cell3 = tf.contrib.rnn.GRUBlockCell(cell_size=FLAGS.state_size)  # ,input_size=FLAGS.embedding_size)

        FQuestion, _   = tf.nn.dynamic_rnn(cell0,inputs=x,dtype=tf.float32,scope='question_forward')
        FContext, _    = tf.nn.dynamic_rnn(cell1,inputs=y,dtype=tf.float32,scope='context_forward')
        RQuestion, _   = tf.nn.dynamic_rnn(cell2,inputs=tf.reverse_sequence(x,self.question_length,seq_axis=1,batch_axis=0),
                                                        dtype=tf.float32,scope='question_backward')
        RContext, _    = tf.nn.dynamic_rnn(cell3,inputs=tf.reverse_sequence(y,self.context_length,seq_axis=1,batch_axis=0),
                                                            dtype=tf.float32,scope='context_backward')

        self.RawQuestion = tf.concat([FQuestion,tf.reverse_sequence(RQuestion,self.question_length,
                            seq_axis=1,batch_axis=0)],axis=2)*tf.expand_dims(self.question_mask_ph,axis=2) # (b,t,2h)
        self.RawContext = tf.concat([FContext,tf.reverse_sequence(RContext,self.context_length,
                            seq_axis=1,batch_axis=0)],axis=2)*tf.expand_dims(self.context_mask_ph,axis=2) # (b,t,2h)

        #========================================================================#
                                    # For separation #
        #========================================================================#

        W_q = tf.get_variable('W_q',(FLAGS.state_size*2,FLAGS.state_size),tf.float32,initializer = tf.contrib.layers.xavier_initializer())
        self.weighted_q = tf.reshape(tf.matmul(tf.reshape(self.RawQuestion,(-1,FLAGS.state_size*2)),W_q),
                                    (FLAGS.batch_size,-1,FLAGS.state_size)) # (b,t,h)

        thiscell1= self.ThisCell(FLAGS.state_size,self)
        thiscell2= self.ThisCell(FLAGS.state_size,self)

        H_rf, _ = tf.nn.dynamic_rnn(thiscell1,inputs=self.RawContext,dtype=tf.float32,scope='mix_forward')
        H_rf *= tf.expand_dims(self.context_mask_ph,axis=2)

        H_rr, _ = tf.nn.dynamic_rnn(thiscell2,inputs=tf.reverse_sequence(self.RawContext,seq_lengths=self.context_length,
                                                    seq_axis=1,batch_axis=0),dtype=tf.float32,scope='mix_backward')
        H_rr = tf.reverse_sequence(H_rr,seq_lengths=self.context_length,seq_axis=1,batch_axis=0)*\
                                                    tf.expand_dims(self.context_mask_ph,axis=2)

        self.concat = tf.nn.dropout(tf.concat([H_rf,H_rr],axis=2),dropout_rate) # (b,t,2h)
        ...
        W_r = tf.get_variable('W_r',(FLAGS.state_size*2,FLAGS.state_size),tf.float32,initializer=tf.contrib.layers.xavier_initializer())
        self.weighted_r = tf.reshape(tf.matmul(tf.reshape(self.concat,(-1,FLAGS.state_size*2)),W_r),
                                     (FLAGS.batch_size,-1,FLAGS.state_size)) # (b,t,h)

        thatcell1 = self.ThatCell(FLAGS.state_size,self)
        thatcell2 = self.ThatCell(FLAGS.state_size,self)

        H_1, _ = tf.nn.dynamic_rnn(thatcell1,inputs=self.concat,dtype=tf.float32,scope='self_correct_forward')
        H_1 *= tf.expand_dims(self.context_mask_ph,axis=2)
        H_2, _ = tf.nn.dynamic_rnn(thatcell2,inputs=tf.reverse_sequence(self.concat,self.context_length,seq_axis=1,batch_axis=0
                                ),dtype=tf.float32,scope='self_correct_backward')
        H_2 = tf.reverse_sequence(H_2,self.context_length,seq_axis=1,batch_axis=0)*tf.expand_dims(self.context_mask_ph,axis=2)

        self_corrected = tf.nn.dropout(tf.concat([H_1,H_2],axis=2),dropout_rate) # (b,t,2h)
        ...
        with tf.variable_scope('final_answer'):
            ...
            U_final = tf.get_variable('U_final',(FLAGS.state_size*2,FLAGS.state_size*2),tf.float32,initializer=tf.contrib.layers.xavier_initializer())
            v1_sfm = tf.get_variable('v1_sfm',(FLAGS.state_size*2,),tf.float32,initializer = tf.contrib.layers.xavier_initializer())
            v2_sfm = tf.get_variable('v2_sfm',(FLAGS.state_size*2,),tf.float32,initializer = tf.contrib.layers.xavier_initializer())

            #comprehension = self.concat # (b,t,2h)
            comprehension = self_corrected
            #comprehension = tf.reshape(tf.matmul(tf.reshape(self_corrected,(-1,FLAGS.state_size*2)),
            #                                     U_final),(FLAGS.batch_size,-1,FLAGS.state_size*2)) # (b,t,2h)
            ...
            W_condition = tf.get_variable('W_condition',(FLAGS.state_size*2,FLAGS.state_size*2),tf.float32,initializer=tf.contrib.layers.xavier_initializer())
            condition = tf.reduce_sum(comprehension*tf.expand_dims(log1,axis=2),axis=1) # the conditions on the start of the answers, (b,2h)
    ...
    def setup_train_op(self):
        ...
        grads, _ = zip(*optn.compute_gradients(self.loss))
    ...
    def setup_embeddings(self):
        ...
        with vs.variable_scope("embeddings"):
            #self.embedding = tf.get_variable(name='embedding',initializer = self.embed)
